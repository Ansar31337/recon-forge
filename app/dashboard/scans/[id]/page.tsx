...
import CveCard from "@/components/scans/CveCard";
...
export default function EnterpriseScanDetail() {
  const { id } = useParams();
  ...
  const [cveData, setCveData] = useState<PageData<Record<string, unknown>>>({
    items: [],
    pagination: { currentPage: 1, limit: 20, totalItems: 0, totalPages: 1 },
  });
  ...
  const [cveLimit, setCveLimit] = useState(20);
  ...
  loadScanRef.current = async () => {
    ...
    p.set("cvePage", String(cvePage));
    p.set("cveLimit", String(cveLimit));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            {scan.enableCveMatching ? (
              <Badge variant="yellow">CVE Enabled</Badge>
            ) : null}
          </div>
        </div>
      ...
      <ExportPanel
        ...
        monthlyDownloadLimit={Number(credits.monthlyDownloadLimit || 1000000)}
      />
      ...
      <SectionBlock
        title="CVE Matches"
        count={counts.cveMatches || 0}
        show={showCve}
        onToggle={() => setShowCve(!showCve)}
        limit={cveLimit}
        onLimitChange={(v) => {
          setCveLimit(v);
          setCvePage(1);
        ...
        page={cvePage}
        totalPages={cveData.pagination?.totalPages || 1}
        onPageChange={setCvePage}
        emptyMsg="No CVE matches found."
      >
        <div className="grid gap-2">
          {cveData.items.map((cve) => (
            <CveCard key={String(cve.id)} cve={cve as any} />
          ))}
        </div>
      </SectionBlock>
    </div>
